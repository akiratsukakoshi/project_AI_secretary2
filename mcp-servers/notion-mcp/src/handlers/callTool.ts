...
export const callToolHandler = async (req: Request, res: Response) => {
  try {
    ...
    try {
      switch (toolName) {
        ...
        case 'createPage':
          const newPage = await notionClient.pages.create(params);
        ...
        case 'deletePage':
          const deletedPage = await notionClient.pages.update({
            page_id: params.page_id,
            archived: true
          });
